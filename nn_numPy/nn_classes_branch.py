...
class NN_FC_Layer:
    ...
    def __init__(self, num_neurons, W, name, alpha = 1e-3, b= None): 
    ...
    def back_propagation(self, X, previous_delta, previous_grad, next_z1 = None):
        ...
        self.dW = previous_grad / N + self.alpha * self.W 
        self.db = np.mean(previous_delta, axis=0)


    def gradient_descent(self, ETA, GAMMA, EPS): 
        '''
        ...
        # #Update Weights
        # gW = GAMMA*gW + (1-GAMMA)*np.sum(self.dW**2) + self.reg
        # eta_W = ETA/np.sqrt(gW + EPS)
        # self.W -= eta_W * self.dW

        # #Update Biases
        # gb = GAMMA*gb + (1-GAMMA)*np.sum(self.db**2)
        # eta_b = ETA/np.sqrt(gb + EPS)            
    ...
    def L2_reg(self): 
        ...
        ''' 
        self.reg = np.sum(self.W**2, axis = 0)
...
class NN_Output_Layer: 
    ...
    def softmax(self, X, test = 0):
        ...
        # Subtract the maximum value for each row for normalisation
